...
def creeInterface(a_largeur = LARGEUR_FEN, a_hauteur = HAUTEUR_FEN) :
    ...
    # Bouton 'Démarrer'
    Button(algoFrame, text='Démarrer', command=demarrerCmd).grid(row=2, column=0)
    # Scale 'Replay'
...
def resizeWindow(evt) :
    global NewLargeur, NewHauteur
    # On sauve les valeurs des événements resize de l'utilisateur (générés par Wroot)
    if evt.widget == Wroot and InitTurtle :
        if evt.width != Wlargeur or evt.height != Whauteur + HAUTEUR_MENU :
            if Debug : print(evt.width, evt.height)
            NewLargeur = evt.width
            NewHauteur = evt.height - HAUTEUR_MENU
    return
